...
- **Doelgroep per onderwijsprogramma** (`education_programs.audience`, seed-stap 72): `children` toont het Hifdh-formulier hieronder voor dat programma (`quran_registrations.program_slug/program_title` onthouden het programma; de letters-vraag staat per programma aan/uit via `require_letters_check`). `adults` toont `components/registration/AdultRegistrationForm.tsx` (voornaam, achternaam, telefoon, e-mail, leeftijd; validatie in `lib/adultRegistration.ts`), dat naar `api/onderwijs/volwassenen` post en één record in `registrations` (type `education`) schrijft. Leeg = volwassenen, behalve Hifdh (`programAudience` in `lib/educationRoutes.ts`). Het oude ouder+studenten-formulier (`RegistrationForm` type `education`, education-tak van `api/inschrijven`) wordt niet meer getoond.
